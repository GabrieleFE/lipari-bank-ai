...
async def test_categorize_utilities_keyword(fake_llm_provider: FakeLLMProvider) -> None:
...
async def test_categorize_groceries_keyword(fake_llm_provider: FakeLLMProvider) -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post(
            "/api/ai/categorize",
